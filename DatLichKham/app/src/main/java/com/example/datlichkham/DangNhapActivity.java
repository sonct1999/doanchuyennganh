...
public class DangNhapActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mappingView();
        prefs = getSharedPreferences("PREFS", MODE_PRIVATE); // Nhận lấy shared preference instance có tên là "PREF"
        //isFirstLogin = true thì show màn hình giới thiệu
        Boolean isFirstLogin = prefs.getBoolean("ISFIRST", true);
        if(isFirstLogin) {
            prefs.edit().putBoolean("ISFIRST", false).apply(); // Lưu giá trị thông qua apply()
